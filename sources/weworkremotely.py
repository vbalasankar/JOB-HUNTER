"""
WeWorkRemotely RSS source (Tier 2).

Feed: https://weworkremotely.com/categories/remote-{category}-jobs.rss
Parses RSS feed via feedparser.
"""

from __future__ import annotations

import logging
from datetime import datetime, timezone

import feedparser
import httpx

from config import get_settings
from sources.base import (
    BaseJobSource,
    RawJob,
    clean_html,
    extract_salary_range,
    safe_parse_date,
)
from storage.models import Job

logger = logging.getLogger(__name__)

# Categories relevant to backend/data engineering
RSS_FEEDS = [
    "https://weworkremotely.com/categories/remote-back-end-programming-jobs.rss",
    "https://weworkremotely.com/categories/remote-full-stack-programming-jobs.rss",
    "https://weworkremotely.com/categories/remote-devops-sysadmin-jobs.rss",
]


class WeWorkRemotelySource(BaseJobSource):
    """Fetches jobs from WeWorkRemotely RSS feeds."""

    name = "weworkremotely"

    async def fetch(self, client: httpx.AsyncClient) -> list[RawJob]:
        all_jobs: list[RawJob] = []

        for feed_url in RSS_FEEDS:
            try:
                resp = await client.get(
                    feed_url,
                    timeout=get_settings().request_timeout,
                    headers={"User-Agent": get_settings().user_agent},
                )
                resp.raise_for_status()

                feed = feedparser.parse(resp.text)
                for entry in feed.entries:
                    # Use the link as a stable ID since RSS doesn't have numeric IDs
                    link = entry.get("link", "")
                    entry_id = entry.get("id", link)

                    all_jobs.append(
                        RawJob(
                            source_id=entry_id,
                            source_name=self.name,
                            raw_data=dict(entry),
                            url=link,
                        )
                    )

                logger.info(
                    f"[weworkremotely] Fetched {len(feed.entries)} jobs from {feed_url}"
                )

            except Exception as e:
                logger.warning(f"[weworkremotely] Failed for {feed_url}: {e}")

        return all_jobs

    def normalize(self, raw: RawJob) -> Job:
        d = raw.raw_data

        title_full = d.get("title", "")
        # WWR format is usually "Company: Title"
        if ":" in title_full:
            company, title = title_full.split(":", 1)
            company = company.strip()
            title = title.strip()
        else:
            company = ""
            title = title_full

        description = clean_html(d.get("summary", "") or d.get("description", ""))
        salary_min, salary_max = extract_salary_range(description)

        # Parse published date
        published = d.get("published") or d.get("updated")

        return Job(
            id=f"{self.name}:{raw.source_id}",
            source=self.name,
            title=title,
            company=company,
            location="Remote",
            remote_type="remote",
            url=raw.url,
            description_raw=description,
            posted_date=safe_parse_date(published),
            salary_min=salary_min,
            salary_max=salary_max,
            fetched_at=datetime.now(timezone.utc),
        )
