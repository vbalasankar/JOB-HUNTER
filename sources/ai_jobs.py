...
Fetches AI/ML-focused jobs from ai-jobs.net via RSS feed.
"""
...
RSS_URL = "https://ai-jobs.net/feed/"


class AIJobsSource(BaseJobSource):
    """Fetches AI/ML jobs from ai-jobs.net RSS."""
    ...
    async def fetch(self, client: httpx.AsyncClient) -> list[RawJob]:
        ...
        try:
            ...
            logger.info(f"[ai_jobs] Fetched {len(feed.entries)} jobs")

        except Exception as e:
            logger.warning(f"[ai_jobs] Failed: {e}")
    ...
    def normalize(self, raw: RawJob) -> Job:
        ...
        # Extract location from tags or description
        tags = [t.get("term", "") for t in d.get("tags", [])]
        location = ""
        for tag in tags:
            if tag.lower() in ("remote", "worldwide"):
                location = "Remote"
                break
            if any(
                c in tag.lower() for c in ("usa", "europe", "uk", "india", "global")
            ):
                location = tag
                break
